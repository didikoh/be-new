...
try {
    if (!$phone || !$course_id || $head_count <= 0) {
        respond(false, "phone, course_id, and head_count are required");
    }

    // 查 user
    $stmt = $pdo->prepare("SELECT id FROM user_list WHERE phone = :phone AND state != -1");
    ...
    // 查 student
    $stmt = $pdo->prepare("SELECT id FROM student_list WHERE user_id = :user_id");
    $stmt->execute([':user_id' => $user_id]);
    $student = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$student) respond(false, "No student found");
