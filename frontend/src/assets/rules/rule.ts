...
export const rules_zh = `1.套餐可以与他人共享，但积分只属于注册会员本人。
\n2.积分只发放给通过App注册的会员（不适用于现场报名或通过WhatsApp注册的会员）。
\n3.过期的套餐无法延长有效期。
\n4.如果会员一年内没有充值，会员资格将被取消。
\n5.会员如需终止套餐，每人有一次机会将剩余余额转让给其他人。
\n6.无论共享预约多少人，该课程的积分仅按一人的课程价格计算。`

export const rules_en = `1. Packages can be shared with others, but points only belong to the registered member.
\n2. Points are only awarded to members who register through the app (not applicable for walk-in members or those who sign up via WhatsApp).
\n3. Expired packages cannot be extended.
\n4. Membership will be cancelled if there is no top-up within one year.
\n5. Members who wish to terminate their package will have one opportunity to transfer their remaining balance to another person.
\n6. Regardless of how many people are booked using a shared package, the points awarded for each class will be based on the price of one person only.
`;
